...
OriginalFreqGainLAF::OriginalFreqGainLAF(){};
OriginalFreqGainLAF::~OriginalFreqGainLAF(){};

void OriginalFreqGainLAF::drawLinearSlider (juce::Graphics& g, int x, int y, int width, int height,
                       float sliderPos,
                       float minSliderPos,
                       float maxSliderPos,
...

{
    ...
    auto trackWidthOriginalBackground = juce::jmin (19.5f, (float) height * 10.f );
    ...
    juce::Path trackBackground;
    trackBackground.startNewSubPath (pos1);
    trackBackground.lineTo (pos2);
    
    g.setColour (juce::Colours::ghostwhite.darker(0.075f));
    g.strokePath (trackBackground, { trackWidthOriginalBackground, juce::PathStrokeType::curved, juce::PathStrokeType::rounded });
    
    //==============================================================================
                                            //Creates Track for Slider
    //==============================================================================
    
    auto trackWidthOriginal = juce::jmin (8.5f, (float) height );
    ...
    g.setColour (offWhite.brighter(5.5f));
                                    g.drawRoundedRectangle (juce::Rectangle<float> (static_cast<float> (x ? maxThumbWidth : minThumbWidth),
                                                                                    ...
                                                                                    .withCentre (x ? maxPoint : minPoint), 325, 10.5);
